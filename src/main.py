...
Example Usage:
  ...
  - Encrypt a file:
    $ menc encrypt -m models/mil_512/config.json -f filename

  - Encrypt from stdin:
    $ echo 'Hello World!' | menc encrypt -m models/mil_512/config.json

  - Encrypt from stdin, key provided as arg:
    $ echo 'Hello World!' | menc encrypt -m models/mil_512/config.json -k foo

  - Store encrypted result into a file:
    $ echo 'Hello World!' | menc encrypt -m models/mil_512/config.json > encrypted_file

  - Decrypt a file:
    $ menc decrypt -m models/mil_512/config.json -f filename

  - Decrypt from stdin:
    $ cat encrypted | menc decrypt -m models/mil_512/config.json

  - Decrypt from stdin, key provided as arg:
    $ cat encrypted | menc decrypt -m models/mil_512/config.json -k foo

  - Store decrypted result into a file:
    $ cat encrypted | menc -m models/mil_512/config.json > decrypted_file

  - Round-trip (encrypt and then decrypt):
    $ echo 'Hello world!' | menc encrypt -m models/mil_512/config.json -k foo | menc decrypt -m models/mil_512/config.json -k foo""")
    subparsers = parser.add_subparsers()

    encrypt_parser = subparsers.add_parser('encrypt', help="Encrypt a plaintext.")
    encrypt_parser.add_argument('-m', '--model', metavar="MODEL_PATH", help="Path to the model config.", required=True)
    ...
    decrypt_parser.add_argument('-m', '--model', metavar="MODEL_PATH", help="Path to the model config.", required=True)
